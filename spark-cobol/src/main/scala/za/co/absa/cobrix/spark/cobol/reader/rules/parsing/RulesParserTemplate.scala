...
private[rules] abstract class RulesParserTemplate {
  ...
  def parse(rulesExpressions: Seq[RuleExpression]): Seq[Rule] = {
    ...
    if (rulesExpressions == null) {
      throw new IllegalArgumentException("Invalid input: expected Seq[RuleExpression] received null")
    }

    rulesExpressions.map(parseRuleExpression(_))
  }

  def parseRuleExpression(ruleExpression: RuleExpression): Rule
}
