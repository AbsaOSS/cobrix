...
# COBOL Data Source for Apache Spark

Pain free Spark/Cobol files integration.

Seamlessly query your Cobol binary files as Spark Dataframes and streams.   
...
2. Start a ```sqlContext.read``` operation specifying ```za.co.absa.spark.cobol.source``` as the format
...
Below is an example whose full version can be found at ```za.co.absa.spark.cobol.examples.SampleApp```
...
Below is an example whose full version can be found at ```za.co.absa.spark.cobol.examples.StreamingExample```
