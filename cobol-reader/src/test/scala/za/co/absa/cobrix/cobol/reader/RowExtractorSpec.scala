...
class RowExtractorSpec extends FunSuite {
  ...
  class Row(val values: Array[Any]) extends RowType[Row] {
    def toSeq = values.toSeq
  }

  def rowCreator: Array[Any] => Row = (values: Array[Any]) => new Row(values)

  test("Test row extractor") {
    val row = RowExtractors.extractRecord(copybook.ast, bytes, startOffset, rowCreate = rowCreator)
    // [[6,[EXAMPLE4,0,],[,,3,[Vector([000000000000002000400012,0,], [000000000000003000400102,1,], [000000005006001200301000,2,])]]]]
    val innerRow = row.head.asInstanceOf[Row].values.asInstanceOf[Array[Any]]

    // id
    assert(innerRow.head.asInstanceOf[Int] === 6)

    // short name
    assert(innerRow(1).asInstanceOf[Row].values.head.asInstanceOf[String] === "EXAMPLE4")

    // company id
    assert(innerRow(1).asInstanceOf[Row].values(1).asInstanceOf[Int] === 0)

    // number of accounts
    assert(innerRow(2).asInstanceOf[Row].values(2).asInstanceOf[Int] === 3)

    // account detail
    val accounts: Array[Any] = innerRow(2).asInstanceOf[Row].values(3).asInstanceOf[Row].values(0).asInstanceOf[Array[Any]]
    val account: Row = accounts(0).asInstanceOf[Row]

    // account number
    assert(account.values(0).asInstanceOf[String] === "000000000000002000400012")

    //account type
    assert(account.values(1).asInstanceOf[Int] === 0)
  }
}
